...
static func _create_god_of_war_task(difficulty: int) -> DivineBlessingTask:
	...
	match difficulty:
		0:  # 簡單
			...
			task.blessing_modifiers = [
				StatModifier.new("damage_bonus", "add", 0.15)
			]
		...
		1:  # 標準
			...
			task.blessing_modifiers = [
				StatModifier.new("damage_bonus", "add", 0.30)
			]
			task.condition_params = {
				"action_tags": ["attack"],
				"min_count": 3,
		...
		2:  # 困難
			...
			task.blessing_modifiers = [
				StatModifier.new("damage_bonus", "add", 0.50)
			]
			task.condition_params = {
				"action_tags": ["attack"],
				"min_percentage": 0.5
...
static func _create_goddess_of_wisdom_task(difficulty: int) -> DivineBlessingTask:
	...
	match difficulty:
		0:  # 簡單
			...
			task.blessing_modifiers = [
				StatModifier.new("damage_reduction", "add", 0.12)
			]
			task.condition_params = {
				"action_tags": ["defend"],
				"min_count": 1,
		...
		2:  # 困難
			...
			task.blessing_modifiers = [
				StatModifier.new("damage_reduction", "add", 0.40)
			]
			task.condition_params = {
				"action_tags": ["defend"],
				"min_success": 10
...
static func _create_god_of_commerce_task(difficulty: int) -> DivineBlessingTask:
	...
	match difficulty:
		0:  # 簡單
			...
			task.blessing_modifiers = [
				StatModifier.new("stamina_recovery_speed", "multiply", 1.20)
			]
			task.condition_params = {
				"unique_actions": 3,
				"window_turns": 5
		...
		2:  # 困難
			...
			task.blessing_modifiers = [
				StatModifier.new("stamina_cost_reduction", "multiply", 0.40)
			]
			task.condition_params = {
...
static func _create_goddess_of_life_task(difficulty: int) -> DivineBlessingTask:
	...
	match difficulty:
		0:  # 簡單
			...
			task.blessing_modifiers = [
				StatModifier.new("hp_recovery_per_turn", "add", 5.0)
			]
			task.condition_params = {
		...
		1:  # 標準
			...
			task.blessing_modifiers = [
				StatModifier.new("hp_recovery_per_turn", "add", 15.0)
			]
			task.condition_params = {
		...
		2:  # 困難
			...
			task.blessing_modifiers = [
				StatModifier.new("hp_recovery_per_turn", "add", 30.0)
			]
			task.condition_params = {
...
static func _create_god_of_deceit_task(difficulty: int) -> DivineBlessingTask:
	...
	match difficulty:
		0:  # 簡單
			...
			task.blessing_modifiers = [
				StatModifier.new("action_speed_bonus", "add", 1.0)
			]
			task.condition_params = {
				"stance_changes": 1,
				"window_turns": 3
		...
		1:  # 標準
			...
			task.blessing_modifiers = [
				StatModifier.new("dodge_on_hit", "add", 0.30)
			]
			task.condition_params = {
		...
		2:  # 困難
			...
			task.blessing_modifiers = [
				StatModifier.new("enemy_accuracy_reduction", "add", 0.30)
			]
			task.condition_params = {
				"max_same_actions": 2,
				"check_window": 5
	...
	match difficulty:
		...
		1:  # 標準
			...
			task.blessing_modifiers = [
				...
				StatModifier.new("damage_reduction", "add", 0.15)
			]
			task.condition_params = {
		...
		2:  # 困難
			...
			task.blessing_modifiers = [
				StatModifier.new("all_damage_bonus", "multiply", 0.25),
				StatModifier.new("all_defense_bonus", "multiply", 0.25)
			]
			task.condition_params = {
